...
export async function getRepoStatus(
  repoPath: string,
  distro: string = 'Ubuntu'
): Promise<{ branch: string; clean: boolean; ahead: number; behind: number }> {
  try {
    ...
    const clean = lines.slice(1).every(l => !l.trim());
    return {
      branch,
      clean,
    ...
    return { branch: 'unknown', clean: true, ahead: 0, behind: 0 };
  }
}
